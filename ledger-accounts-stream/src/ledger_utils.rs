...
const PROCESS_SLOTS_HELP_STRING: &str = "The starting slot is either the latest found snapshot slot, or genesis (slot 0) if the \
...
pub fn load_and_process_ledger(
    ...
    let (snapshot_request_sender, _) = crossbeam_channel::unbounded();
    ...
    .map(|_| LoadAndProcessLedgerOutput { bank_forks })
